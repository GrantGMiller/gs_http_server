...
class HTTP_Server:
    ...
    def route(self, *args, **kwargs):
        def decorator(f):
            route_pattern = self.build_route_pattern(args[0])
    ...
    def DataProcess(self, client, data):
        ...
        request = self.normalize_line_endings(data)  # hack again
        ...
        try:
            response, code = self.serve(request_uri, request_method, request_body)
        except TypeError:
            response, code = json.dumps({'Error': 'Bad Request'}), 404
    ...
    def DataReturn(self, client, data):
        ...
        try:
            ...
            client.Send(
                '\r\n')  # to separate headers from body (https://stackoverflow.com/questions/5757290/http-header-line-break-style#5757349)
    ...
    def _InitServerEvents(self):
        ...
        def HandleReceiveData(client, data):
            self.print('HandleReceiveData({}, {}'.format(client, data.decode()))
